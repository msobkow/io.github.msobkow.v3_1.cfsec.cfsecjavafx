// Description: Java 25 JavaFX Attribute Pane implementation for SecSysGrp.
...
/**
 *	CFSecJavaFXSecSysGrpAttrPane JavaFX Attribute Pane implementation
 *	for SecSysGrp.
 */
public class CFSecJavaFXSecSysGrpAttrPane
extends CFGridPane
implements ICFSecJavaFXSecSysGrpPaneCommon
{
		...
		extends CFLabel
	{
		public NameCFLabel() {
			super();
			setText(Inz.s("cfsec.javafx.SecSysGrp.AttrPane.Name.EffLabel"));
		}
	}
		...
		extends CFStringEditor
	{
		public NameEditor() {
			super();
			setMaxLen( 64 );
			setFieldNameInzTag( "cfsec.javafx.SecSysGrp.AttrPane.Name.EffLabel" );
		}
	}

	protected class SecLevelCFLabel
		extends CFLabel
	{
		public SecLevelCFLabel() {
			super();
			setText(Inz.s("cfsec.javafx.SecSysGrp.AttrPane.SecLevel.EffLabel"));
		}
	}

	protected class SecLevelEditor
		extends ComboBox<String>
	{
		public SecLevelEditor() {
			super();
			setItems( observableListOfSecLevel );
		}
	}
	...
	protected SecLevelEditor javafxEditorSecLevel = null;

	public CFSecJavaFXSecSysGrpAttrPane( ICFFormManager formManager, ICFSecJavaFXSchema argSchema, ICFSecSecSysGrpObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
		...
		label = getJavaFXLabelSecLevel();
		setHalignment( label, HPos.LEFT );
		setValignment( label, VPos.BOTTOM );
		add( label, 0, gridRow );
		gridRow ++;

		ctrl = getJavaFXEditorSecLevel();
		setHalignment( ctrl, HPos.LEFT );
		add( ctrl, 0, gridRow );
		gridRow ++;
	...
	public ICFSecSecSysGrpObj getEffJavaFXFocus() {
		ICFSecSecSysGrpObj eff = (ICFSecSecSysGrpObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFSecSecSysGrpObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	public SecLevelCFLabel getJavaFXLabelSecLevel() {
		if( javafxLabelSecLevel == null ) {
			javafxLabelSecLevel = new SecLevelCFLabel();
		}
		return( javafxLabelSecLevel );
	}

	public void setJavaFXLabelSecLevel( SecLevelCFLabel value ) {
		javafxLabelSecLevel = value;
	}

	public SecLevelEditor getJavaFXEditorSecLevel() {
		if( javafxEditorSecLevel == null ) {
			javafxEditorSecLevel = new SecLevelEditor();
		}
		return( javafxEditorSecLevel );
	}

	public void setJavaFXEditorSecLevel( SecLevelEditor value ) {
		javafxEditorSecLevel = value;
	}

	public void populateFields()
	{
		ICFSecSecSysGrpObj popObj = getEffJavaFXFocus();
		...
		if( popObj == null ) {
			getJavaFXEditorSecLevel().setValue( null );
		}
		else {
			getJavaFXEditorSecLevel().setValue(
				( popObj.getRequiredSecLevel() == null )
					? null
					: popObj.getRequiredSecLevel().toString() );
		}
	}
	...
	public void postFields()
	{
		...
		ICFSecSecSysGrpObj focus = getJavaFXFocusAsSecSysGrp();
		ICFSecSecSysGrpEditObj editObj;
		if( focus != null ) {
			editObj = (ICFSecSecSysGrpEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		String strseclevel = getJavaFXEditorSecLevel().getValue();
		ICFSecSchema.SecLevelEnum curseclevel = ICFSecSchema.parseSecLevelEnum( "SecLevel", strseclevel );
		editObj.setRequiredSecLevel( curseclevel );
	}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFSecSecSysGrpEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFSecSecSysGrpObj)editObj.create();
	...
	public void adjustComponentEnableStates() {
		...
		if( javafxEditorSecLevel != null ) {
			javafxEditorSecLevel.setDisable( ! isEditing );
		}
	}
}
