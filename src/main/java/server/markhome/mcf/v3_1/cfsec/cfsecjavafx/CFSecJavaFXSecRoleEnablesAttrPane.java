// Description: Java 25 JavaFX Attribute Pane implementation for SecRoleEnables.
...
/**
 *	CFSecJavaFXSecRoleEnablesAttrPane JavaFX Attribute Pane implementation
 *	for SecRoleEnables.
 */
public class CFSecJavaFXSecRoleEnablesAttrPane
extends CFGridPane
implements ICFSecJavaFXSecRoleEnablesPaneCommon
{
	protected ICFFormManager cfFormManager = null;
	protected ICFSecJavaFXSchema javafxSchema = null;
	boolean javafxIsInitializing = true;

	protected class SecRoleEnablesGroupCFLabel
		extends CFLabel
	{
		public SecRoleEnablesGroupCFLabel() {
			super();
			setText(Inz.s("cfsec.javafx.SecRoleEnables.AttrPane.ParentEnableGroup.EffLabel"));
		}
	}

	protected class CallbackSecRoleEnablesGroupChosen
	implements ICFSecJavaFXSecSysGrpChosen
	{
		public CallbackSecRoleEnablesGroupChosen() {
		}

		public void choseSecSysGrp( ICFSecSecSysGrpObj value ) {
			if( javafxReferenceParentEnableGroup != null ) {
				ICFSecSecRoleEnablesObj cur = getJavaFXFocusAsSecRoleEnables();
				if( cur != null ) {
					ICFSecSecRoleEnablesEditObj editObj = (ICFSecSecRoleEnablesEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	protected class SecRoleEnablesGroupReferenceCallback
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFSecSecRoleEnablesObj focus = getEffJavaFXFocus();
			...
			CFBorderPane form = javafxSchema.getSecSysGrpFactory().newPickerForm( cfFormManager, referencedObj, null, cltn, new CallbackSecRoleEnablesGroupChosen() );
	...
	protected class SecRoleEnablesGroupCFReferenceEditor
		extends CFReferenceEditor
	{
		public SecRoleEnablesGroupCFReferenceEditor() {
			super( new SecRoleEnablesGroupReferenceCallback() );
			setFieldNameInzTag( "cfsec.javafx.SecRoleEnables.AttrPane.SecRoleEnablesGroup.EffLabel" );
		}
	}

	protected ICFSecSecSysGrpObj javafxParentEnableGroupObj = null;
	protected SecRoleEnablesGroupCFLabel javafxLabelParentEnableGroup = null;
	protected SecRoleEnablesGroupCFReferenceEditor javafxReferenceParentEnableGroup = null;

	public CFSecJavaFXSecRoleEnablesAttrPane( ICFFormManager formManager, ICFSecJavaFXSchema argSchema, ICFSecSecRoleEnablesObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
		...
		setJavaFXFocusAsSecRoleEnables( argFocus );
	...
	public ICFSecSecRoleEnablesObj getEffJavaFXFocus() {
		ICFSecSecRoleEnablesObj eff = (ICFSecSecRoleEnablesObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFSecSecRoleEnablesObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	public CFLabel getJavaFXLabelParentEnableGroup() {
		if( javafxLabelParentEnableGroup == null ) {
			javafxLabelParentEnableGroup = new SecRoleEnablesGroupCFLabel();
		}
	...
	public CFReferenceEditor getJavaFXReferenceParentEnableGroup() {
		if( javafxReferenceParentEnableGroup == null ) {
			javafxReferenceParentEnableGroup = new SecRoleEnablesGroupCFReferenceEditor();
		}
		return( javafxReferenceParentEnableGroup );
	}

	public void setJavaFXReferenceParentEnableGroup( SecRoleEnablesGroupCFReferenceEditor value ) {
		javafxReferenceParentEnableGroup = value;
	}

	public void populateFields()
	{
		ICFSecSecRoleEnablesObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFSecSecRoleEnablesObj focus = getJavaFXFocusAsSecRoleEnables();
		ICFSecSecRoleEnablesEditObj editObj;
		if( focus != null ) {
			editObj = (ICFSecSecRoleEnablesEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFSecSecRoleEnablesEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFSecSecRoleEnablesObj)editObj.create();
