// Description: Java 25 JavaFX Attribute Pane implementation for SecRole.
...
/**
 *	CFSecJavaFXSecRoleAttrPane JavaFX Attribute Pane implementation
 *	for SecRole.
 */
public class CFSecJavaFXSecRoleAttrPane
extends CFGridPane
implements ICFSecJavaFXSecRolePaneCommon
{
	...
	protected class SecRoleIdCFLabel
		extends CFLabel
	{
		public SecRoleIdCFLabel() {
			super();
			setText(Inz.s("cfsec.javafx.SecRole.AttrPane.SecRoleId.EffLabel"));
		}
	}

	protected class SecRoleIdEditor
		extends CFDbKeyHash256Editor
	{
		public SecRoleIdEditor() {
			super();
			setFieldNameInzTag( "cfsec.javafx.SecRole.AttrPane.SecRoleId.EffLabel" );
		}
	}
		...
		extends CFLabel
	{
		public NameCFLabel() {
			super();
			setText(Inz.s("cfsec.javafx.SecRole.AttrPane.Name.EffLabel"));
		}
	}
		...
		extends CFStringEditor
	{
		public NameEditor() {
			super();
			setMaxLen( 64 );
			setFieldNameInzTag( "cfsec.javafx.SecRole.AttrPane.Name.EffLabel" );
		}
	}

	protected SecRoleIdCFLabel javafxLabelSecRoleId = null;
	protected SecRoleIdEditor javafxEditorSecRoleId = null;
	protected NameCFLabel javafxLabelName = null;
	protected NameEditor javafxEditorName = null;

	public CFSecJavaFXSecRoleAttrPane( ICFFormManager formManager, ICFSecJavaFXSchema argSchema, ICFSecSecRoleObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
		...
		int gridRow = 0;
		label = getJavaFXLabelSecRoleId();
		setHalignment( label, HPos.LEFT );
		setValignment( label, VPos.BOTTOM );
		add( label, 0, gridRow );
		gridRow ++;

		ctrl = getJavaFXEditorSecRoleId();
	...
	public ICFSecSecRoleObj getEffJavaFXFocus() {
		ICFSecSecRoleObj eff = (ICFSecSecRoleObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFSecSecRoleObj)eff.getEdit();
			}
		}
		return( eff );
	}

	public SecRoleIdCFLabel getJavaFXLabelSecRoleId() {
		if( javafxLabelSecRoleId == null ) {
			javafxLabelSecRoleId = new SecRoleIdCFLabel();
		}
		return( javafxLabelSecRoleId );
	}

	public void setJavaFXLabelSecRoleId( SecRoleIdCFLabel value ) {
		javafxLabelSecRoleId = value;
	}

	public SecRoleIdEditor getJavaFXEditorSecRoleId() {
		if( javafxEditorSecRoleId == null ) {
			javafxEditorSecRoleId = new SecRoleIdEditor();
		}
		return( javafxEditorSecRoleId );
	}

	public void setJavaFXEditorSecRoleId( SecRoleIdEditor value ) {
		javafxEditorSecRoleId = value;
	}
	...
	{
		ICFSecSecRoleObj popObj = getEffJavaFXFocus();
		if( getPaneMode() == CFPane.PaneMode.Unknown ) {
			popObj = null;
		}
		if( popObj == null ) {
			getJavaFXEditorSecRoleId().setDbKeyHash256Value( null );
		}
		else {
			getJavaFXEditorSecRoleId().setDbKeyHash256Value( popObj.getRequiredSecRoleId() );
		}
	...
	public void postFields()
	{
		...
		ICFSecSecRoleObj focus = getJavaFXFocusAsSecRole();
		ICFSecSecRoleEditObj editObj;
		if( focus != null ) {
			editObj = (ICFSecSecRoleEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFSecSecRoleEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFSecSecRoleObj)editObj.create();
	...
	public void adjustComponentEnableStates() {
		...
		if( javafxEditorSecRoleId != null ) {
			javafxEditorSecRoleId.setDisable( true );
		}
		if( javafxEditorName != null ) {
