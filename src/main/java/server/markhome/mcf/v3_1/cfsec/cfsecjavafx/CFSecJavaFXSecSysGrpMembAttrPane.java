// Description: Java 25 JavaFX Attribute Pane implementation for SecSysGrpMemb.
...
/**
 *	CFSecJavaFXSecSysGrpMembAttrPane JavaFX Attribute Pane implementation
 *	for SecSysGrpMemb.
 */
public class CFSecJavaFXSecSysGrpMembAttrPane
extends CFGridPane
implements ICFSecJavaFXSecSysGrpMembPaneCommon
{
	...
	protected class SecSysGrpMembUserCFLabel
		extends CFLabel
	{
		public SecSysGrpMembUserCFLabel() {
			super();
			setText(Inz.s("cfsec.javafx.SecSysGrpMemb.AttrPane.ParentUser.EffLabel"));
		}
	}

	protected class CallbackSecSysGrpMembUserChosen
	implements ICFSecJavaFXSecUserChosen
	{
		public CallbackSecSysGrpMembUserChosen() {
		}

		public void choseSecUser( ICFSecSecUserObj value ) {
			if( javafxReferenceParentUser != null ) {
				ICFSecSecSysGrpMembObj cur = getJavaFXFocusAsSecSysGrpMemb();
				if( cur != null ) {
					ICFSecSecSysGrpMembEditObj editObj = (ICFSecSecSysGrpMembEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFSecJavaFXSecUserPageCallback
	{
		...
		public List<ICFSecSecUserObj> pageData( CFLibDbKeyHash256 priorSecUserId )
		{
			...
			ICFSecSecSysGrpMembObj focus = (ICFSecSecSysGrpMembObj)getEffJavaFXFocus();
			if( focus != null ) {
	...
	protected class SecSysGrpMembUserReferenceCallback
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			Node cont;
			ICFSecSchemaObj schemaObj = (ICFSecSchemaObj)javafxSchema.getSchema();
			ICFSecSecSysGrpMembObj focus = getEffJavaFXFocus();
			ICFSecSecUserObj referencedObj = (ICFSecSecUserObj)javafxReferenceParentUser.getReferencedObject();
			CFBorderPane form = javafxSchema.getSecUserFactory().newPickerForm( cfFormManager, referencedObj, null, new PageDataParentUserList(), new CallbackSecSysGrpMembUserChosen() );
	...
	protected class SecSysGrpMembUserCFReferenceEditor
		extends CFReferenceEditor
	{
		public SecSysGrpMembUserCFReferenceEditor() {
			super( new SecSysGrpMembUserReferenceCallback() );
			setFieldNameInzTag( "cfsec.javafx.SecSysGrpMemb.AttrPane.SecSysGrpMembUser.EffLabel" );
		}
	}

	protected ICFSecSecUserObj javafxParentUserObj = null;
	protected SecSysGrpMembUserCFLabel javafxLabelParentUser = null;
	protected SecSysGrpMembUserCFReferenceEditor javafxReferenceParentUser = null;

	public CFSecJavaFXSecSysGrpMembAttrPane( ICFFormManager formManager, ICFSecJavaFXSchema argSchema, ICFSecSecSysGrpMembObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
		...
		setJavaFXFocusAsSecSysGrpMemb( argFocus );
	...
	public ICFSecSecSysGrpMembObj getEffJavaFXFocus() {
		ICFSecSecSysGrpMembObj eff = (ICFSecSecSysGrpMembObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFSecSecSysGrpMembObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	public CFLabel getJavaFXLabelParentUser() {
		if( javafxLabelParentUser == null ) {
			javafxLabelParentUser = new SecSysGrpMembUserCFLabel();
		}
	...
	public CFReferenceEditor getJavaFXReferenceParentUser() {
		if( javafxReferenceParentUser == null ) {
			javafxReferenceParentUser = new SecSysGrpMembUserCFReferenceEditor();
		}
		return( javafxReferenceParentUser );
	}

	public void setJavaFXReferenceParentUser( SecSysGrpMembUserCFReferenceEditor value ) {
		javafxReferenceParentUser = value;
	}

	public void populateFields()
	{
		ICFSecSecSysGrpMembObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFSecSecSysGrpMembEditObj editObj;
		if( focus != null ) {
			editObj = (ICFSecSecSysGrpMembEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFSecSecSysGrpMembEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFSecSecSysGrpMembObj)editObj.create();
