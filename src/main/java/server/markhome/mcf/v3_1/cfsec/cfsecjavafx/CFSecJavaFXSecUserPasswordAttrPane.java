// Description: Java 25 JavaFX Attribute Pane implementation for SecUserPassword.
...
/**
 *	CFSecJavaFXSecUserPasswordAttrPane JavaFX Attribute Pane implementation
 *	for SecUserPassword.
 */
public class CFSecJavaFXSecUserPasswordAttrPane
extends CFGridPane
implements ICFSecJavaFXSecUserPasswordPaneCommon
{
		...
		extends CFLabel
	{
		public PWSetStampCFLabel() {
			super();
			setText(Inz.s("cfsec.javafx.SecUserPassword.AttrPane.PWSetStamp.EffLabel"));
		}
	}
		...
		extends CFTimestampEditor
	{
		public PWSetStampEditor() {
			super();
			setFieldNameInzTag( "cfsec.javafx.SecUserPassword.AttrPane.PWSetStamp.EffLabel" );
		}
	}
		...
		extends CFLabel
	{
		public PasswordHashCFLabel() {
			super();
			setText(Inz.s("cfsec.javafx.SecUserPassword.AttrPane.PasswordHash.EffLabel"));
		}
	}
		...
		extends CFStringEditor
	{
		public PasswordHashEditor() {
			super();
			setMaxLen( 256 );
			setFieldNameInzTag( "cfsec.javafx.SecUserPassword.AttrPane.PasswordHash.EffLabel" );
		}
	}
	...
	public CFSecJavaFXSecUserPasswordAttrPane( ICFFormManager formManager, ICFSecJavaFXSchema argSchema, ICFSecSecUserPasswordObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
		...
		setJavaFXFocusAsSecUserPassword( argFocus );
	...
	public ICFSecSecUserPasswordObj getEffJavaFXFocus() {
		ICFSecSecUserPasswordObj eff = (ICFSecSecUserPasswordObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFSecSecUserPasswordObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFSecSecUserPasswordObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFSecSecUserPasswordObj focus = getJavaFXFocusAsSecUserPassword();
		ICFSecSecUserPasswordEditObj editObj;
		if( focus != null ) {
			editObj = (ICFSecSecUserPasswordEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		editObj.setRequiredPWSetStamp( getJavaFXEditorPWSetStamp().getTimestampValue() );
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFSecSecUserPasswordEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFSecSecUserPasswordObj)editObj.create();
