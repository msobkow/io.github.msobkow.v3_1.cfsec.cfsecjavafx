// Description: Java 25 JavaFX List of Obj Pane implementation for SecSysGrpInc.
...
/**
 *	CFSecJavaFXSecSysGrpIncListPane JavaFX List of Obj Pane implementation
 *	for SecSysGrpInc.
 */
public class CFSecJavaFXSecSysGrpIncListPane
extends CFBorderPane
implements ICFSecJavaFXSecSysGrpIncPaneList
{
	public static String S_FormName = "List System Security Group Include";
	...
	protected CFButton buttonAddSecSysGrpInc = null;
	...
	public CFSecJavaFXSecSysGrpIncListPane( ICFFormManager formManager,
		...
		boolean sortByChain )
	{
		...
		if( observableListOfSecSysGrpInc != null ) {
	...
	public CFHBox getPanelHBoxMenu() {
		if( hboxMenu == null ) {
			...
			buttonAddSecSysGrpInc = new CFButton();
			buttonAddSecSysGrpInc.setMinWidth( 200 );
			buttonAddSecSysGrpInc.setText( "Add System Security Group Include" );
			buttonAddSecSysGrpInc.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					final String S_ProcName = "handle";
					try {
						ICFSecSchemaObj schemaObj = (ICFSecSchemaObj)javafxSchema.getSchema();
						ICFSecSecSysGrpIncObj obj = (ICFSecSecSysGrpIncObj)schemaObj.getSecSysGrpIncTableObj().newInstance();
						ICFSecSecSysGrpIncEditObj edit = (ICFSecSecSysGrpIncEditObj)( obj.beginEdit() );
						if( edit == null ) {
							throw new CFLibNullArgumentException( getClass(),
								S_ProcName,
								0,
						...
						CFBorderPane frame = javafxSchema.getSecSysGrpIncFactory().newAddForm( cfFormManager, obj, getViewEditClosedCallback(), true );
						ICFSecJavaFXSecSysGrpIncPaneCommon jpanelCommon = (ICFSecJavaFXSecSysGrpIncPaneCommon)frame;
			...
			hboxMenu.getChildren().add( buttonAddSecSysGrpInc );
				...
				@Override public void handle( ActionEvent e ) {
					...
					try {
						...
						if( selectedInstance != null ) {
							...
							if( entry.getSchemaName().equals("CFSec") && backingClassCode == ICFSecSecSysGrpInc.CLASS_CODE ) {
								CFBorderPane frame = javafxSchema.getSecSysGrpIncFactory().newViewEditForm( cfFormManager, selectedInstance, getViewEditClosedCallback(), false );
								((ICFSecJavaFXSecSysGrpIncPaneCommon)frame).setPaneMode( CFPane.PaneMode.View );
								cfFormManager.pushForm( frame );
							}
							else {
								throw new CFLibUnsupportedClassException( getClass(),
				...
				@Override public void handle( ActionEvent e ) {
					...
					try {
						...
						if( selectedInstance != null ) {
							...
							if( entry.getSchemaName().equals("CFSec") && backingClassCode == ICFSecSecSysGrpInc.CLASS_CODE ) {
								CFBorderPane frame = javafxSchema.getSecSysGrpIncFactory().newViewEditForm( cfFormManager, selectedInstance, getViewEditClosedCallback(), false );
								((ICFSecJavaFXSecSysGrpIncPaneCommon)frame).setPaneMode( CFPane.PaneMode.Edit );
								cfFormManager.pushForm( frame );
							}
							else {
								throw new CFLibUnsupportedClassException( getClass(),
				...
				@Override public void handle( ActionEvent e ) {
					...
					try {
						...
						if( selectedInstance != null ) {
							...
							int backingClassCode = entry.getBackingClassCode();
							if( entry.getSchemaName().equals("CFSec") && backingClassCode == ICFSecSecSysGrpInc.CLASS_CODE ) {
								CFBorderPane frame = javafxSchema.getSecSysGrpIncFactory().newAskDeleteForm( cfFormManager, selectedInstance, getDeleteCallback() );
								((ICFSecJavaFXSecSysGrpIncPaneCommon)frame).setPaneMode( CFPane.PaneMode.View );
								cfFormManager.pushForm( frame );
							}
							else {
	...
	public void adjustListButtons() {
		...
		if( buttonAddSecSysGrpInc != null ) {
			buttonAddSecSysGrpInc.setDisable( ! allowAdds );
		}
