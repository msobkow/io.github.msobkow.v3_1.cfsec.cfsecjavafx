// Description: Java 25 JavaFX Attribute Pane implementation for SecRoleMemb.
...
/**
 *	CFSecJavaFXSecRoleMembAttrPane JavaFX Attribute Pane implementation
 *	for SecRoleMemb.
 */
public class CFSecJavaFXSecRoleMembAttrPane
extends CFGridPane
implements ICFSecJavaFXSecRoleMembPaneCommon
{
	protected ICFFormManager cfFormManager = null;
	protected ICFSecJavaFXSchema javafxSchema = null;
	boolean javafxIsInitializing = true;

	protected class SecRoleMembUserCFLabel
		extends CFLabel
	{
		public SecRoleMembUserCFLabel() {
			super();
			setText(Inz.s("cfsec.javafx.SecRoleMemb.AttrPane.ParentUser.EffLabel"));
		}
	}

	protected class CallbackSecRoleMembUserChosen
	implements ICFSecJavaFXSecUserChosen
	{
		public CallbackSecRoleMembUserChosen() {
		}

		public void choseSecUser( ICFSecSecUserObj value ) {
			if( javafxReferenceParentUser != null ) {
				ICFSecSecRoleMembObj cur = getJavaFXFocusAsSecRoleMemb();
				if( cur != null ) {
					ICFSecSecRoleMembEditObj editObj = (ICFSecSecRoleMembEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFSecJavaFXSecUserPageCallback
	{
		...
		public List<ICFSecSecUserObj> pageData( CFLibDbKeyHash256 priorSecUserId )
		{
			...
			ICFSecSecRoleMembObj focus = (ICFSecSecRoleMembObj)getEffJavaFXFocus();
			if( focus != null ) {
	...
	protected class SecRoleMembUserReferenceCallback
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			Node cont;
			ICFSecSchemaObj schemaObj = (ICFSecSchemaObj)javafxSchema.getSchema();
			ICFSecSecRoleMembObj focus = getEffJavaFXFocus();
			ICFSecSecUserObj referencedObj = (ICFSecSecUserObj)javafxReferenceParentUser.getReferencedObject();
			CFBorderPane form = javafxSchema.getSecUserFactory().newPickerForm( cfFormManager, referencedObj, null, new PageDataParentUserList(), new CallbackSecRoleMembUserChosen() );
	...
	protected class SecRoleMembUserCFReferenceEditor
		extends CFReferenceEditor
	{
		public SecRoleMembUserCFReferenceEditor() {
			super( new SecRoleMembUserReferenceCallback() );
			setFieldNameInzTag( "cfsec.javafx.SecRoleMemb.AttrPane.SecRoleMembUser.EffLabel" );
		}
	}

	protected ICFSecSecUserObj javafxParentUserObj = null;
	protected SecRoleMembUserCFLabel javafxLabelParentUser = null;
	protected SecRoleMembUserCFReferenceEditor javafxReferenceParentUser = null;

	public CFSecJavaFXSecRoleMembAttrPane( ICFFormManager formManager, ICFSecJavaFXSchema argSchema, ICFSecSecRoleMembObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFSecSecRoleMembObj getEffJavaFXFocus() {
		ICFSecSecRoleMembObj eff = (ICFSecSecRoleMembObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFSecSecRoleMembObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	public CFLabel getJavaFXLabelParentUser() {
		if( javafxLabelParentUser == null ) {
			javafxLabelParentUser = new SecRoleMembUserCFLabel();
		}
	...
	public CFReferenceEditor getJavaFXReferenceParentUser() {
		if( javafxReferenceParentUser == null ) {
			javafxReferenceParentUser = new SecRoleMembUserCFReferenceEditor();
		}
		return( javafxReferenceParentUser );
	}

	public void setJavaFXReferenceParentUser( SecRoleMembUserCFReferenceEditor value ) {
		javafxReferenceParentUser = value;
	}

	public void populateFields()
	{
		ICFSecSecRoleMembObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFSecSecRoleMembObj focus = getJavaFXFocusAsSecRoleMemb();
		ICFSecSecRoleMembEditObj editObj;
		if( focus != null ) {
			editObj = (ICFSecSecRoleMembEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFSecSecRoleMembEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFSecSecRoleMembObj)editObj.create();
